	...
	<div class="form-container">
		<form method="post" enctype="multipart/form-data">
			...
			<div class="image-upload-section">
				...
				<div class="image-grid" id="files">
					<c:forEach var="image" items="${item.itemImages}">
						<div class="image-item existing-file">
							...
							<div class="image-controls">
								<label class="main-image-label">
									...
									<c:if test="${image.sortOrder == 1}">checked</c:if>>
									<span>대표</span>
								</label>
								<button type="button" class="delete-image-btn deleteImage" data-code="${image.id}">삭제</button>
							</div>
						</div>
					</c:forEach>
				</div>
			...
			<div class="form-group">
				...
				<input type="text" id="title" name="title" class="form-control" value="${item.title}" placeholder="상품명을 입력해주세요" required>
			</div>
			
			<!-- 상품 설명 -->
			<div class="form-group">
				<label for="description">상품 설명 <span class="required">*</span></label>
				<textarea id="description" name="description" class="form-control" placeholder="상품에 대해 자세히 설명해주세요" required>${item.description}</textarea>
			...
			<div class="form-group">
				...
				<input type="text" id="delivery" name="delivery" class="form-control" value="${item.delivery}" placeholder="예: 서울특별시 강남구" required>
			</div>
			...
			<div class="form-group">
				...
				<div class="radio-group">
					<label class="radio-label">
						...
						<c:if test="${item.isDirectDeal == 'N' or empty item.isDirectDeal}">checked</c:if>>
						<span>직거래 불가</span>
					</label>
					<label class="radio-label">
						<input type="radio" name="isDirectDeal" value="Y" 
						<c:if test="${item.isDirectDeal == 'Y'}">checked</c:if>>
						<span>직거래 가능</span>
					</label>
				</div>
			</div>
			...
			<div class="form-group">
				...
				<input type="text" id="bankAccountNumber" name="bankAccountNumber" class="form-control" value="${item.bankAccountNumber}" placeholder="예: 국민 123-456-789012" required>
			...
			<div class="form-row">
				<div class="form-group">
					...
					<input type="number" id="price" name="price" class="form-control" value="${item.price}" placeholder="0" min="0" required>
				</div>
				
				<div class="form-group">
					<label for="amount">판매 수량 (개) <span class="required">*</span></label>
					<input type="number" id="amount" name="amount" class="form-control" value="${item.amount}" placeholder="0" min="1" required>
				</div>
			</div>
