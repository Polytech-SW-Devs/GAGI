...
public abstract class BaseBoardController<T> {
	...
	@GetMapping("/list")
	public String list(@RequestParam(defaultValue = "") String search, @RequestParam(defaultValue = "1") int page,
			Model model) {
	...
	/*
	 * @PostMapping("/delete/{id}") public String delete(@PathVariable int id) {
	 * service.delete(id); return "redirect:/" + viewPath; }
	 */
